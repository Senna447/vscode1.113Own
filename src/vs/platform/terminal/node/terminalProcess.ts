	...
	async start(): Promise<ITerminalLaunchError | ITerminalLaunchResult | undefined> {
		...
		try {
			...
			if (errorMessage?.includes('Cannot launch conpty')) {
				return { message: localize('conptyLaunchFailed', "ConPTY launch failed. Please set `{0}` to false in settings to use WinPTY compatibility mode.", 'terminal.integrated.windowsEnableConpty') };
			}
